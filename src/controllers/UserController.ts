...
export default class UserController {
  ...
  async login (request: Request, response: Response): Promise<Response<any, Record<string, any>>> {
    ...
    try {
      ...
      if (await bcrypt.compare(password, user.password)) {
        const token = jwt.sign(user.id.toString(), process.env.ACCESS_TOKEN_SECRET ?? '')
